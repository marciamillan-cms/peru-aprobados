...
├── auth.py               # Simple username/password login
...
auth_salt = "some-random-string"

[credentials]
reviewer1 = "sha256-hash-of-their-password"
```

Generate a password hash for the app's own login screen locally:

```bash
python -c "from auth import hash_password_for_secrets as h; print(h('their-password'))"
```

(`auth_salt` / `[credentials]` are unrelated to Eventtia — they control
who can open this internal app, as a separate login layer.)
